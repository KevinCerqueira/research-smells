...
class Research:
    ...
    def read_amout_sonar_smells_author(self):
        self.dataset.execute("""
            ...
            GROUP BY gc.project_id, gc.author HAVING count(distinct gc.commit_hash) > 6
        """)
    ...
    def read_amout_sonar_smells_project(self):
        self.dataset.execute("""
            ...
            WHERE
                gc.merge = 'False' 
                AND si.type = 'CODE_SMELL'  
            GROUP BY gc.project_id HAVING count(distinct gc.commit_hash) > 6
        """)
    ...
    def read_number_lines_edited_author(self):
        self.dataset.execute("""
            ...
            GROUP BY git_commits.project_id, git_commits.author HAVING count(distinct git_commits.commit_hash) > 6
        """)
    ...
    def read_number_lines_edited_project(self):
        self.dataset.execute("""
            ...
            WHERE
                gc.merge = 'False' 
            GROUP BY gcc.project_id HAVING count(distinct gc.commit_hash) > 6
        """)
    ...
    def delete_commit_diff(self):
        self.local_db.execute("""
            DELETE FROM commit_diff 
            WHERE EXISTS (
            SELECT 1 
            FROM (
                SELECT project_id, author
                FROM commit_diff
                GROUP BY project_id, author
                HAVING COUNT(*) <= 2
            ) AS x
            WHERE commit_diff.project_id = x.project_id AND commit_diff.author = x.author
            );
        """)
        self.conn_local_db.commit()
    ...
    def input_cv(self):
        ...
        df = pd.read_sql_query(query, self.conn_local_db)
        ...
        for index, row in stats_df.iterrows():
            ...
            # self.local_db.execute("INSERT INTO cv (project_id, author, code_smells, lines_edited, commits, sonar_smells, mean, std, cv) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (project_id, author, code_smells, lines_edited, commits, sonar_smells, mean, std, cv))
            self.local_db.execute("INSERT INTO cv (project_id, author, mean, std, cv) VALUES (?, ?, ?, ?, ?)", (project_id, author, mean, std, cv))
            self.conn_local_db.commit()
            self.local_db.execute("select code_smells, lines_edited, commits, sonar_smells from author_percentage_information where project_id = ? and author = ?", (project_id, author))
            result = self.local_db.fetchone()
            self.local_db.execute("update cv set code_smells = ?, lines_edited = ?, commits = ?, sonar_smells = ? where project_id = ? and author = ?", (result[0], result[1], result[2], result[3], project_id, author))
            self.conn_local_db.commit()
